          ...
          <div className="space-y-2">
            {MOCK_ORDERS.filter(o => o.status !== 'completed').concat(
              ...
              >
                ...
                <div className="text-paper-500 text-xs truncate">{o.delivery.address}</div>
        ...
        <div className="md:col-span-2 space-y-4">
          {isNew && (
            <div className="flex items-start gap-3 bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
              ...
              <div>
                <div className="font-semibold text-paper-900 text-sm">訂單已建立！</div>
                ...
                return (
                  <div key={s.status} className="relative flex items-center gap-4 pb-5 last:pb-0">
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center z-10 flex-shrink-0
                      ${done
                        ? 'bg-paper-900'
                        : 'bg-paper-100 border-2 border-paper-300'}`}>
                      {done
                        ? <CheckCircle size={14} className="text-white" />
                        : <Circle size={14} className="text-paper-500" />}
                    </div>
                    <span className={`text-sm font-medium ${current ? 'text-paper-900' : done ? 'text-paper-600' : 'text-paper-400'}`}>
                      {s.label}
                      {current && <span className="ml-2 text-paper-900 text-xs animate-pulse-soft">進行中</span>}
                    </span>
                  </div>
                )
              })}
          ...
          <div className="card space-y-3">
            ...
            <div className="flex gap-4 pt-2">
              ...
              <div className="ml-auto font-bold text-paper-900">NT${order.totalFee}</div>
            </div>
          </div>
        </div>
      </div>
